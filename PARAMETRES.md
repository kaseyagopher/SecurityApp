...
|----------|-------------|------|
| **PC** (serveur + Expo) | `10.78.217.97` | API port **3001**, Metro port **8081** |
| **ESP32** (porte) | `10.78.217.47` | HTTP port **80** |
...
const char* BACKEND_HOST    = "10.78.217.97";    // IP du PC (ipconfig)
...
```ts
const FALLBACK_HOST = '10.78.217.97';  // IP du PC
```
...
Même Wi‑Fi que le PC → Expo Go → QR ou `exp://10.78.217.97:8081`.
...
| 2 | PC | http://10.78.217.47/status | JSON capteur |
| 3 | **Téléphone** (Chrome) | http://10.78.217.97:3001/api/health | même JSON |
...
Puis retester sur le téléphone : `http://10.78.217.97:3001/api/health`
